...
io.sockets.on('connection', function (socket) {
  ...
  socket.on("posting_update", function (data){
      console.log('we are in posting_update');

      connectionCount++;

      io.emit('updated_count', {count: connectionCount});
      //socket.emit('random_number', {number:15} );
  })

  socket.on("posting_reset", function (data){
      console.log('we are in posting_reset');
